...
class EntanglerDemo(artiq_env.EnvExperiment):
    ...
    Uses the 4-output/4-input DIO-card mapping generated by device_db.py:
    ttl0..ttl3 are Entangler outputs, ttl4..ttl7 are Entangler inputs, and
    entangler0 is the Entangler driver.
    """

    def build(self):
        """Add the Entangler driver."""
        self.setattr_device("core")
        self.entangler = self.get_device("entangler0")
        self.entangle_outputs = [
            self.get_device("ttl{}".format(i)) for i in range(num_outputs)
        ]
        self.entangle_inputs = [
            self.get_device("ttl{}".format(num_outputs + i))
    ...
    def init(self):
        ...
        # Marker signal for observing timing while the Entangler output passthrough is disabled.
        self.entangle_outputs[0].pulse(1.5 * aq_units.us)
    ...
    def run_entangler(self, timeout_length: TInt32):
        ...
        with parallel:
            ...
            for ttl_input in self.entangle_inputs:
                ttl_input.gate_rising_mu(numpy.int64(timeout_length))
